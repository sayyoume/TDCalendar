...
class CComboExWnd : public CWindowWnd
{
public:
...
void CComboExWnd::Init(CComboExUI* pOwner)
{
    m_pOwner = pOwner;
    m_pLayout = NULL;
...
LPCTSTR CComboExWnd::GetWindowClassName() const
{
    return _T("ComboWnd");
}

void CComboExWnd::OnFinalMessage(HWND hWnd)
{
...
LRESULT CComboExWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    if( uMsg == WM_CREATE ) {
        m_pm.Init(m_hWnd);
		...
		case  VK_UP:
		{
			...
			if (m_iOldSel <= 0)
				m_iOldSel = 1;
			if (m_iOldSel >= m_pOwner->GetCount())
				m_iOldSel = m_pOwner->GetCount()-1;
...
void CComboExWnd::EnsureVisible(int iIndex)
{
	if (iIndex < 0) iIndex = 1;
	if (iIndex >= m_pOwner->GetCount()) iIndex = m_pOwner->GetCount()-1;
...
void CComboExWnd::Scroll(int dx, int dy)
{
    if( dx == 0 && dy == 0 ) return;
...
#if(_WIN32_WINNT >= 0x0501)
UINT CComboExWnd::GetClassStyle() const
{
...
CComboExUI::CComboExUI() : m_pWindow(NULL), m_iCurSel(-1), m_uButtonState(0)
{
	...
	m_ListInfo.dwSelectedBkColor = 0xFFFDF4BF;
    m_ListInfo.dwHotTextColor = 0xFF000000;
	m_ListInfo.dwHotBkColor = 0xFFFDF4BF;
...
{
	if( _tcscmp(pstrName, DUI_CTR_COMBO) == 0 ) return static_cast<CComboExUI*>(this);
...
CDuiString CComboExUI::GetText() const
{
...
CDuiString CComboExUI::GetDropBoxAttributeList()
{
