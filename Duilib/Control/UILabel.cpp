...
namespace DuiLib
{
	...
	void CLabelUI::PaintText(HDC hDC)
	{
		...
		{
			if (m_TextValue.IsEmpty()) m_TextValue = m_sText;
			...
			if (dwTextColor < 0xFF000000)
			{
				Color cr(dwTextColor);
				nGraphics.DrawString(m_TextValue, m_TextValue.GetLength(), &nFont, nRc, &format, &SolidBrush(cr));
				return;
			}
