		...
		if( event.Type == UIEVENT_MOUSEMOVE )
		{
			if( (m_uThumbState & UISTATE_CAPTURED) != 0 ) {
				if( !m_bHorizontal ) {
					...
					int cyRemain = (m_rcItem.bottom - m_rcItem.top) - (m_rcThumb.bottom - m_rcThumb.top) - (m_rcButton1.bottom - m_rcButton1.top) - (m_rcButton2.bottom - m_rcButton2.top);
					if (cyRemain == 0)
						cyRemain = 1;
					m_nLastScrollOffset = m_nRange * (event.ptMouse.y - ptLastMouse.y) / cyRemain;
				}
				else {
					...
					int cxRemain = (m_rcItem.right - m_rcItem.left) - (m_rcThumb.right - m_rcThumb.left) - (m_rcButton1.right - m_rcButton1.left) - (m_rcButton2.right - m_rcButton2.left);
					if (cxRemain == 0)
						cxRemain = 1;
					m_nLastScrollOffset = (event.ptMouse.x - ptLastMouse.x) * m_nRange / cxRemain;
				}
			}
			else {
