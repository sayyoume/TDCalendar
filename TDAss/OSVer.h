...
#define WINMOD_WIN7_SP0     0x06010000
	...
	public:
		...
		static bool  GetOSVersion(UGOSVersion& osType)
		{
			...
			BOOL bRetState;
			bRetState = ::GetVersionEx(reinterpret_cast<LPOSVERSIONINFO>(&osVerInfo));
			if (false == bRetState)
			{
				return true;
			}
			...
			{
			case 5:
				if (1 == osVerInfo.dwMinorVersion)
				{
					osType = WIN_XP;
				}
				else if ((2 == osVerInfo.dwMinorVersion) &&
					(osVerInfo.wProductType == VER_NT_WORKSTATION) &&
					(systemInfo.wProcessorArchitecture == PROCESSOR_ARCHITECTURE_AMD64))//WinXP 64bit
				{
					osType = WIN_XP;
				}
				else if ((2 == osVerInfo.dwMinorVersion && GetSystemMetrics(SM_SERVERR2) == 0) ||
					(2 == osVerInfo.dwMinorVersion && GetSystemMetrics(SM_SERVERR2) != 0))//Win2003 R2	
				{
					osType = WIN_2003;
				}
				else
			...
			case 6:
				if (osVerInfo.dwMinorVersion == 0 && osVerInfo.wProductType == VER_NT_WORKSTATION)
				{
					osType = WIN_VISTA;
				}
				else if ((osVerInfo.dwMinorVersion == 0 && osVerInfo.wProductType != VER_NT_WORKSTATION) ||
					(osVerInfo.dwMinorVersion == 1 && osVerInfo.wProductType != VER_NT_WORKSTATION)
					)
				{
					osType = WIN_2008;
				}
				else if (osVerInfo.dwMinorVersion == 1 && osVerInfo.wProductType == VER_NT_WORKSTATION)
				{
					osType = WIN_7;
				}
				else if ((osVerInfo.dwMinorVersion == 2 && osVerInfo.wProductType != VER_NT_WORKSTATION) ||
					(osVerInfo.dwMinorVersion == 3 && osVerInfo.wProductType != VER_NT_WORKSTATION))
				{
					osType = WIN_2012;
				}
				else if (osVerInfo.dwMinorVersion == 2 && osVerInfo.wProductType == VER_NT_WORKSTATION)
				...
				else if (osVerInfo.dwMinorVersion == 3 && osVerInfo.wProductType == VER_NT_WORKSTATION)
				{
					osType = WIN_8_1;
				}
				else
		...
		*/
		...
		static bool getSystemVersion(DWORD& dwMajor,DWORD& dwMinor,DWORD& dwSP)
		{
		...
		static ULONG GetRealProcAddress(HANDLE hModule, char *lpProcName)
		{
		...
		static BOOL MyGetVersion(PRTL_OSVERSIONINFOEXW pOSVersion)
		{
			if(pOSVersion==NULL)
				return FALSE;
	...
	#define STATUS_SUCCESS	0
		...
		static BOOL GDP_GetVersionExW(LPOSVERSIONINFOW lpVersionInfo)
		{
		...
		static BOOL GetMiniOSVersionDesc(LPWSTR lpDesc,DWORD dwDescLength)
		{
