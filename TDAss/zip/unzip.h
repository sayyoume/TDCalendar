...
APDDLL_DECLSPEC HZIP OpenZip(const TCHAR *fn, const char *password);
APDDLL_DECLSPEC HZIP OpenZip(void *z, unsigned int len, const char *password);
APDDLL_DECLSPEC HZIP OpenZipHandle(HANDLE h, const char *password);
...
APDDLL_DECLSPEC ZRESULT GetZipItem(HZIP hz, int index, ZIPENTRY *ze);
...
APDDLL_DECLSPEC ZRESULT FindZipItem(HZIP hz, const TCHAR *name, bool ic, int *index, ZIPENTRY *ze);
// FindZipItem - finds an item by name. ic means 'insensitive to case'.
// It returns the index of the item, and returns information about it.
// If nothing was found, then index is set to -1 and the function returns
// an error code.

APDDLL_DECLSPEC ZRESULT UnzipItem(HZIP hz, int index, const TCHAR *fn);
APDDLL_DECLSPEC ZRESULT UnzipItem(HZIP hz, int index, void *z, unsigned int len);
APDDLL_DECLSPEC ZRESULT UnzipItemHandle(HZIP hz, int index, HANDLE h);
...
APDDLL_DECLSPEC ZRESULT SetUnzipBaseDir(HZIP hz, const TCHAR *dir);
// if unzipping to a filename, and it's a relative filename, then it will be relative to here.
// (defaults to current-directory).


APDDLL_DECLSPEC ZRESULT CloseZip(HZIP hz);
...
APDDLL_DECLSPEC ZRESULT CloseZipU(HZIP hz);
